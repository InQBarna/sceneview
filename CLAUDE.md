...
- ✅ **Render Tests CI FIXED** (PR #814, closes #803): 4 render test classes re-enabled — `@Ignore` removed, Engine-sharing fix applied (per-class shared Engine instead of per-test). GeometryRenderTest, VisualVerificationTest, LightingRenderTest, RenderSmokeTest all running in CI.
